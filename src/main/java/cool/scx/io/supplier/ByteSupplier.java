...
public interface ByteSupplier extends AutoCloseable {
    ...
    /// - 为了防止 异常冲突 get 不允许抛出任何易混淆异常 如 AlreadyClosedException.
    ByteChunk get() throws ScxIOException;

    /// - 如果持有底层资源, 在此方法释放资源
    /// - 注意这里和 ByteInput 的 close 语义不同, 作为最底层的 数据提供器, 此处需要允许多次调用 (幂等)
    /// - 为了防止 异常冲突 close 不允许抛出任何易混淆异常 如 AlreadyClosedException.
    @Override
    default void close() throws ScxIOException {
